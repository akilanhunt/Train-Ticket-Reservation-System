	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException {
		...
		try {
			...
			if (trains != null && !trains.isEmpty()) {
				...
				rd.include(req, res);
				pw.println("<div class='main'><p1 class='menu'>Fare for Trains BetWeen Station " + ESAPI.encoder().encodeForHTML(fromStation) + " and "
						+ ESAPI.encoder().encodeForHTML(toStation) + " is as below</p1></div>");
				...
				rd.include(req, res);
				pw.println("<div class='tab'><p1 class='menu'>There are no trains Between " + ESAPI.encoder().encodeForHTML(fromStation) + " and "
						+ ESAPI.encoder().encodeForHTML(toStation) + "</p1></div>");
			}
		} catch (Exception e) {
